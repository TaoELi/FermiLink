...
from fermilink.drvloop.prompts import (
    DRVLOOP_LEGACY_STATE_DIRNAME,
    DRVLOOP_MEMORY_DIRNAME,
    DRVLOOP_MEMORY_FILENAME,
    DRVLOOP_STATE_DIRNAME,
    DRVLOOP_STATE_FILENAME,
)
...
def state_path_for(repo_dir: Path) -> Path:
    return repo_dir / DRVLOOP_STATE_DIRNAME / DRVLOOP_STATE_FILENAME


def legacy_state_path_for(repo_dir: Path) -> Path:
    return repo_dir / DRVLOOP_LEGACY_STATE_DIRNAME / DRVLOOP_STATE_FILENAME
...
def snapshot_project_artifacts(repo_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    memory_rel = f"{DRVLOOP_MEMORY_DIRNAME}/{DRVLOOP_MEMORY_FILENAME}"
...
def record_artifact_changes(repo_dir: Path, _memory_path: Path) -> list[dict[str, Any]]:
