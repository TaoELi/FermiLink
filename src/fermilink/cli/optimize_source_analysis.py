...
def build_benchmark_generation_prompt(
    *,
    ...
    return (
        ...
        "- `correctness`: either `mode: runner_only` (simplest) or `mode: field_tolerances`\n"
        "  with tolerance specs derived from the source analysis output quantities.\n"
        "  Use `field_tolerances` when the analysis identified numeric output quantities\n"
        "  with suggested tolerances.  Otherwise use `runner_only`.\n"
        "- `runtime`:\n"
        "  - `mode: direct`\n"
