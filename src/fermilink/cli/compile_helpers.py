...
def _derive_recompile_paper_skill_id(
    ...
    *,
    ...
    comment_token = _slugify_token(str(comment or ""))
    doc_token = _slugify_token(doc_path.stem)
    if comment_token and comment_token != "paper":
        base_slug = comment_token[:36]
    else:
        base_slug = doc_token[:36]
        ...
        _write_text(
            ...
            "\n".join(
                [
                    ...
                    "",
                    "## Scope",
                    "- Paper reproduction tutorial skill generated by `fermilink recompile --doc ...`.",
                    "",
                    "## High-Signal Playbook",
                    "- Route: Use this skill for manuscript figure reproduction workflows.",
                    "- Triage questions: target figure, parameter regime, data source, acceptance checks.",
                    "- Canonical workflow: select figure -> prepare inputs -> run simulation -> postprocess -> validate.",
                    "- Minimal working example: populate with commands in pass 2/3.",
                    "- Pitfalls: update with manuscript-specific caveats in pass 2/3.",
                    "- Convergence/validation checklist: update with manuscript-specific checks in pass 2/3.",
                ]
            ),
        )
