...
EXPLOOP_AGENTS_FILENAME = "AGENTS.md"
EXPLOOP_AGENTS_ALIAS_FILENAMES = ("CLAUDE.md", "GEMINI.md")
EXPLOOP_BACKUP_SUFFIX = ".pre-exploop-backup"


@dataclass(frozen=True)
class ExploopInstructionFiles:
    agents_path: Path
    alias_paths: tuple[Path, ...]


def materialize_exploop_instructions(repo_dir: Path) -> ExploopInstructionFiles:
    """Copy exploop agent guidance into a workspace without any git setup."""

    repo_dir.mkdir(parents=True, exist_ok=True)
    guide = load_exploop_guide()
    if not guide.strip():
        raise FileNotFoundError("Missing packaged exploop AGENTS.md guide.")

    agents_path = repo_dir / EXPLOOP_AGENTS_FILENAME
    _write_authoritative_file(agents_path, guide)

    alias_paths: list[Path] = []
    for alias_name in EXPLOOP_AGENTS_ALIAS_FILENAMES:
        alias_path = repo_dir / alias_name
        _ensure_agents_alias(agents_path, alias_path)
        alias_paths.append(alias_path)

    return ExploopInstructionFiles(
    ...
    if path.exists() or path.is_symlink():
        ...
        if path.is_dir() and not path.is_symlink():
            raise FileExistsError(
                f"Cannot write exploop AGENTS.md over directory: {path}"
            )
...
def _ensure_agents_alias(agents_path: Path, alias_path: Path) -> None:
    if alias_path.exists() or alias_path.is_symlink():
        ...
        if alias_path.is_file() and _safe_read_text(alias_path) == _safe_read_text(
            ...
            raise FileExistsError(
                f"Cannot write exploop AGENTS.md alias over directory: {alias_path}"
            )
        else:
...
def _next_backup_path(path: Path) -> Path:
    first = path.with_name(path.name + EXPLOOP_BACKUP_SUFFIX)
    if not first.exists() and not first.is_symlink():
        return first
    index = 1
    while True:
        candidate = path.with_name(f"{path.name}{EXPLOOP_BACKUP_SUFFIX}.{index}")
