...
def test_humanize_metric_label_handles_common_optimize_metrics() -> None:
    ...
    assert (
        plot_optimize.humanize_metric_label(
...
def test_build_report_humanizes_default_metric_label(tmp_path: Path) -> None:
    ...
    summary = json.loads(
        (out_dir / "data" / "summary.json").read_text(encoding="utf-8")
    )
...
def test_build_report_copies_benchmark_inputs_and_indexes_them(tmp_path: Path) -> None:
    ...
    assert (out_dir / "inputs" / "all" / "shared.dat").read_text(
        encoding="utf-8"
    ) == "shared\n"
    assert (out_dir / "inputs" / "all" / "sub" / "case.in").read_text(
        encoding="utf-8"
    ) == "case\n"
    ...
    assert (
        "Update ``runtime.pre_commands`` for machine-specific build/setup steps"
        in rerun_section
    )
    ...
    assert index_text.index("Benchmark Contracts") < index_text.index(
        "Input files for Benchmarks"
    )
    assert index_text.index("Input files for Benchmarks") < index_text.index(
        "Runtime Data"
    )
    contract_section = index_text.split(
        "Benchmark Contracts\n-------------------\n", 1
    )[1].split("Input files for Benchmarks\n--------------------------\n", 1)[0]
    assert (
...
def test_github_repo_url_from_remote_handles_common_github_formats() -> None:
    ...
    assert (
        build_report.github_repo_url_from_remote(
            "git@github.com:skilled-scipkg/mockpkg.git"
        )
        == "https://github.com/skilled-scipkg/mockpkg"
    )
    assert (
        build_report.github_repo_url_from_remote(
            "ssh://git@github.com/skilled-scipkg/mockpkg.git"
        )
        == "https://github.com/skilled-scipkg/mockpkg"
    )
    assert (
        build_report.github_repo_url_from_remote(
            "https://github.com/skilled-scipkg/mockpkg.git"
        )
        == "https://github.com/skilled-scipkg/mockpkg"
    )
...
def test_build_report_adds_github_commit_links_when_branch_is_published(
    tmp_path: Path,
) -> None:
    build_report = _load_build_report_module()
    ...
    iter_text = (out_dir / "iterations" / "iter_0001_accepted.rst").read_text(
        encoding="utf-8"
    )
...
def test_push_optimize_branch_creates_remote_branch_with_set_upstream(
    tmp_path: Path,
) -> None:
    build_report = _load_build_report_module()
...
def test_push_optimize_branch_retries_with_default_github_cli_login_for_https_remote(
    ...
    _git(
        ...
        "set-url",
        "origin",
        "https://github.com/example/mockpkg.git",
    )
    optimize_dir = repo_dir / ".fermilink-optimize"
    optimize_dir.mkdir(parents=True, exist_ok=True)
        ...
        if (
            ...
            helper_arg = next(
                (
                    part
                    for part in cmd
                    if isinstance(part, str) and part.startswith("credential.helper=!")
                ),
                None,
            )
            if helper_arg:
    ...
    monkeypatch.setattr(
        build_report.shutil, "which", lambda name: gh_path if name == "gh" else None
    )
    ...
    assert f"credential.helper=!{gh_path} auth git-credential" in second_cmd
